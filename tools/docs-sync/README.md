...
- **Cost.** Copilot engine = flat cost on your existing seats (no metered key).
  ...
  model.)*
- **Docs-only.** The tool allow-list (`write`, `shell(git:*)` — no interpreter
  is allow-listed) lets the agent edit anything in the working tree; what
  actually prevents production-code changes from landing is the post-run
  guard (fails the run if any touched path falls outside
  `AGENTS.md CLAUDE.md doc Docs .docs-sync.json`) plus staging only those
  same paths before the PR is opened.
- **Always reviewed.** Commits land on a `chore/docs-*` branch behind a PR;
  nothing reaches a protected branch unreviewed.
- **Attribution.** Commit identity is the generic `docs-sync` bot, and the agent
  is told to add no attribution markers (`Co-authored-by`, "generated by", tool
  credits). That is a ban on attribution, not on the subject — docs that
  legitimately describe AI tooling (`docs/ai/**`, agent prompts) stay intact. Add the `ai-influenced` label via branch policy
...
# Guard the first-run case: with no `Docs-sync-anchor:` commit in history the
# inner git log yields nothing and `git log ""` errors out, so the HEAD~20
# fallback below is never reached.
DOC_COMMIT=$(git log -1 --format=%H --grep='^Docs-sync-anchor:' || true)
ANCHOR=""
if [ -n "$DOC_COMMIT" ]; then
  ANCHOR=$(git log -1 --format=%B "$DOC_COMMIT" \
